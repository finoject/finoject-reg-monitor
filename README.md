...
**毎時37分（JST・1日24回）自動巡回**し、新着項目（日付・タイトル・リンク）を
...
- `.github/workflows/crawl.yml` … cron（`37 * * * *` = 毎時37分・1日24回。:00はGitHubが混雑しdropしやすいため回避）で巡回 → data.jsonをコミット → GitHub Pagesへ公開。
...
- **全組織を毎時（現状 毎時37分・1日24回）巡回**する。巡回は機関単位ではなくワークフロー単位で、1回の実行で `crawler.js` の `SITES` 全件をループ取得する。
...
3. 以上だけで、毎時の巡回・新規差分検知（同一URLの日付更新も再検知）・Slack通知・一覧表示がすべて自動適用される。
...
## 母艦ヘルスチェック（ops/healthcheck）
毎朝7:00にMac mini母艦上のClaude Codeがヘッドレスで自己点検し、異常時のみSlackへ報告する仕組み。
巡回自体はGitHub Actionsで動くが、母艦側のMCP認証（freee/Gmail）と公開出力の鮮度は誰も見ていないため、そこを埋める検知層。
