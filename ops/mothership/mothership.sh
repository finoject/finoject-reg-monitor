...
# 「抱えたまま」動き続ける。落ちれば launchd の KeepAlive が再起動する。
#
# なぜラッパーが要るか:
#   `claude --remote-control` は *インタラクティブ* 起動でTTYを要求する。
#   launchd の配下にTTYは無いので、擬似端末(pty)を割り当てないと即死する。
#   ターミナルや tmux から手起動していると、再起動・ログアウト・ターミナル終了で
#   母艦ごと消え、bridge セッションが一斉に computer_unreachable になる。これはその対策。
...
# --- 多重起動の回避 ---------------------------------------
# pgrep -f は使わない。`--remote-control` という文字列を含むコマンドラインを
...
# --- pty の割り当て方式を決める ---------------------------
# tmux: 後から `tmux -S <sock> attach` で画面を覗ける（推奨）
# script: macOS標準の /usr/bin/script で pty を作る（brew不要のフォールバック）
...
cleanup() {
  ...
  if [ "$PTY_MODE" = "tmux" ]; then
    tmux -S "$TMUX_SOCKET" kill-session -t "$TMUX_SESSION" 2>/dev/null
...
if [ "$PTY_MODE" = "tmux" ]; then
  tmux -S "$TMUX_SOCKET" kill-session -t "$TMUX_SESSION" 2>/dev/null
  if ! tmux -S "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" -c "$WORKDIR" \
       "$CLAUDE_BIN" --remote-control "$SESSION_NAME"; then
    die_slowly "tmux セッションを起動できなかった。ログ: $LOG_FILE"
  fi
  tmux -S "$TMUX_SOCKET" list-panes -t "$TMUX_SESSION" -F '#{pane_pid}' 2>/dev/null | head -1 > "$PID_FILE"
  log "tmux セッション '$TMUX_SESSION' で起動（覗く: tmux -S $TMUX_SOCKET attach -t $TMUX_SESSION / 抜ける: Ctrl-b d）"
  ...
  /usr/bin/script -q /dev/null "$CLAUDE_BIN" --remote-control "$SESSION_NAME" >/dev/null 2>&1 &
  CHILD_PID=$!
